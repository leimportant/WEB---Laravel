...
class AdendumPadimasController extends Controller
{
	public function index()
    {
     if (! Gate::allows('adendum-padimas.create')) {
            return abort(401);
        }
        $datas = DB::table('adendum_agreement')->wherein('adendum_type', ['P-DIST'])->get();
        $waiting = DB::table('adendum_agreement')->select(DB::raw('count(id) as id'))
                        ->wherein('adendum_type', ['P-DIST'])
                        ->wherein('flag', ['0', '3'])
                        ->get();

        return view('site.group.adendum-padimas.index', compact(['datas', 'waiting']));
    }
    ...
    public function create()
    {
        ...
        $relations = [
             'agreementList0' =>  DB::table('cooperation_agreement')->wherein('company_type',  ['P-DIST'])->pluck('contract_id', 'doc_id')    
                       ->prepend('Please select', ''),   
        ];

        return view('site.group.adendum-padimas.create', $relations);
    }
     ...
     public function store(Request $request)
    {
        ...
        $data = AdendumPadimas::create($request->all());
        Toast::success('Data Successfull', 'info');
        return redirect()->route('adendum-padimas.index');
    }
    ...
    public function edit($id)
    {
        ...
        $data = AdendumPadimas::findOrFail($id);

        return view('site.group.adendum-padimas.edit', compact('data') + $relations);
    }
    ...
    public function view($id)
    {
     if (! Gate::allows('adendum-padimas.create')) {
        ...
        return view('site.group.adendum-padimas.view', compact('data'));
    }
     ...
     public function vPdfagen($id)
    {
       if (! Gate::allows('adendum-padimas.create')) {
        ...
        $content = view('site.group.adendum-padimas.html2pdf_agen', compact(['data']));
      ...
      public function vPdfdistributor($id)
    {
       if (! Gate::allows('adendum-padimas.create')) {
        ...
        $content = view('site.group.adendum-padimas.html2pdf_dist', compact(['data']));
    ...
    public function show()
    {
     if (! Gate::allows('adendum-padimas.create')) {
        ...
        return view('site.group.adendum-padimas.admin', compact(['datas', 'waiting']));
    }
    ...
    {
       if (! Gate::allows('adendum-padimas.admin')) {
            return abort(401);
        }
        $data = AdendumPadimas::findOrFail($id);
         $relations = [
         ...
         return view('site.group.adendum-padimas.approve', compact('data') + $relations);
    }
    ...
    public function viewpdf($id)
    {
      if (! Gate::allows('adendum-padimas.admin')) {
        ...
        $cover =  \View::make('site.group.adendum-padimas.cover_agen', compact('data', 'date', 'cover_agen'))->render();
        $content =  \View::make('site.group.adendum-padimas.agen_full', compact('data', 'agen_full'))->render();
